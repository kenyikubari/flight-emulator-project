...
/// Entry point for controller 2.
///
/// Responsibilities:
/// - Read serial telemetry (altitude, pitch, roll)
/// - Update LCD display
/// - Drive pitch and roll motors using PWM
#[arduino_hal::entry]
fn main() -> ! {
    ...
    // Pitch motor - <Motor 1, Controller 2> (PWM: D10, DIR: D8/D9)
    ...
    // Roll motor - <Motor 2, Controller 2> (PWM: D11, DIR: D13/D12)
        ...
        while let Ok(byte) = serial.read() {
            if byte == b'\n' {
                ...
                if let (Some(alt), Some(p), Some(r)) = (p1, p2, p3) {
                    ...
                    // Update targets
                    altitude_current = alt.trim().parse().unwrap_or(altitude_current);
